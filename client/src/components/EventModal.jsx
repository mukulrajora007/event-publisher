...
export default function EventModal({ event, onClose, onRegisterSuccess, showToast }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto bg-slate-950/80 backdrop-blur-md animate-fadeIn">
      ...
      <div 
        className="relative w-full max-w-3xl bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl my-8"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 p-2 rounded-full bg-slate-950/70 text-slate-400 hover:text-white hover:bg-slate-900 transition"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="relative h-64 sm:h-80 w-full overflow-hidden bg-slate-950">
          ...
          <div className="absolute bottom-6 left-6 right-6 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <span className="px-3 py-1 rounded-full text-xs font-semibold bg-indigo-600/90 text-white backdrop-blur-md shadow-sm">
                {event.category}
              </span>
            ...
            {diffDays > 0 && (
              <span className="px-3 py-1 rounded-full text-xs font-medium bg-slate-950/80 text-slate-300 border border-slate-700/80 backdrop-blur-md">
        ...
        <div className="p-6 sm:p-8 space-y-8 max-h-[calc(85vh-200px)] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 rounded-2xl bg-slate-950/60 border border-slate-800/80">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-xs font-semibold uppercase text-slate-400">Date & Time</div>
            ...
            <div className="flex items-start gap-3">
              <div className="p-2.5 rounded-xl bg-violet-500/10 text-violet-400 border border-violet-500/20">
                {event.location_type === 'virtual' ? <Video className="w-5 h-5" /> : <MapPin className="w-5 h-5" />}
              </div>
              <div className="flex-1">
                ...
                {event.location_url && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1 text-[11px] font-semibold text-indigo-400 hover:text-indigo-300 mt-1 truncate max-w-[200px]"
                  >
          ...
          {/* Description */}
          <div className="space-y-3">
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-300">About this event</h3>
          ...
          {event.tags && event.tags.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {event.tags.map((tag, i) => (
                <span key={i} className="px-2.5 py-1 rounded-md text-xs font-medium bg-slate-800 text-slate-300">
                  #{tag}
                </span>
              ))}
            </div>
          ...
          {/* Organizer Info Box */}
          <div className="p-4 rounded-2xl bg-slate-950/40 border border-slate-800 flex items-center justify-between gap-4">
            <div className="space-y-0.5">
              <div className="text-xs text-slate-400">Organized by</div>
              <div className="text-sm font-bold text-white">{event.organizer_name}</div>
            ...
            {event.organizer_website && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-1 text-xs font-semibold text-indigo-400 hover:text-indigo-300 p-2 rounded-lg bg-slate-800/80 hover:bg-slate-800 transition"
              >
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-br from-indigo-950/40 via-slate-900 to-violet-950/30 border border-indigo-500/20 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold text-white flex items-center gap-2">
                  <Ticket className="w-4 h-4 text-indigo-400" />
                  <span>Register & Reserve Your Spot</span>
                </h3>
                <p className="text-xs text-slate-400 mt-0.5">
                  {event.capacity - (event.attendees_count || 0)} of {event.capacity} spots remaining
                </p>
              </div>
              ...
              <div className="text-right">
                ...
                <div className="text-[11px] text-slate-400">Per attendee</div>
              </div>
            </div>

            {registered ? (
              <div className="p-4 rounded-xl bg-emerald-950/50 border border-emerald-800/60 flex items-center gap-3">
                <CheckCircle2 className="w-6 h-6 text-emerald-400 flex-shrink-0" />
                <div>
                  <div className="text-sm font-bold text-emerald-300">You are officially registered!</div>
              ...
              <form onSubmit={handleRegister} className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 text-sm bg-slate-950 border border-slate-800 rounded-lg text-white focus:outline-none focus:border-indigo-500"
                    />
                  </div>
                ...
                <div className="flex items-center justify-between pt-2">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-500 transition disabled:opacity-50 shadow-md shadow-indigo-600/30"
                  >
